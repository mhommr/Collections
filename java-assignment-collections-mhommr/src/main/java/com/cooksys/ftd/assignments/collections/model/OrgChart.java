...
public class OrgChart {
    ...
    public boolean addEmployee(Employee employee) {
        ...
        Manager currManager = employee.getManager();
        while (currManager != null) {
        	orgChart.add(employee);
        	orgChart.add(currManager);
        	currManager = currManager.getManager();
       } 
       if (employee instanceof Manager) {
    	   orgChart.add(employee);
       }
       return hasEmployee(employee);
    }
    ...
    public Set<Manager> getAllManagers() {
        ...
        for (Employee worker : getAllEmployees()) {
        	if (worker instanceof Manager) {
        		managerSet.add((Manager)worker);
        	}
        }
        return managerSet;
    }
    ...
    public Set<Employee> getDirectSubordinates(Manager manager) {
        ...
        for (Employee emp : getAllEmployees()) {
        	Manager currMan = emp.getManager();
        	if (currMan != null && currMan.equals(manager)) {
        		subordinateSet.add(emp);
        	}
...
//        	}
    ...
    public Map<Manager, Set<Employee>> getFullHierarchy() {
    	Set<Manager> managers = new HashSet<>(getAllManagers());
    	Map<Manager, Set<Employee>> hierarchyMap = new HashMap<>();
    	for (Manager m : managers) {
    		hierarchyMap.put(m, this.getDirectSubordinates(m));
    	}
    	return hierarchyMap;
    }
